...
int child_handle(int pipefd)
{
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            recv(new_fd,password,sizeof(password),0);

            sp=getspnam(username);//get encrypted password through username
            ERROR_CHECK(sp,NULL,"getspnam");

            get_salt(salt,sp->sp_pwdp);

            if(strcmp(sp->sp_pwdp,crypt(password,salt)) == 0)//correct username and password
            {
                send(new_fd,"1",1,0);//send 1 on behalf of succuss
                break;
            }
            else
            {
                send(new_fd,"0",1,0);//send 0 on behalf of failure
            }
        }
...
int do_gets(int new_fd,char *cmd)
{
    ...
    else
    {
        ...
        send(new_fd,&file_len,8,0);//send the length of the file to choice the right transmission methed

        if(file_len < 100*1024)//less than 100M 
        {
            //receive the qualification command to testify wheather there is a identical file
            recv(new_fd,&file_len,8,0);
            if(0 != file_len)
            {
        ...
        else//more than 100M
        {
            ...
            if(0 != file_size)
            {
                ...
                char * p = (char *)mmap(NULL,file_len-file_size,PROT_READ,MAP_SHARED,fd,0);
                ERROR_CHECK(p,(char *)-1,"mmap");
                    
                ret = send(new_fd,p,file_len-file_size,MSG_NOSIGNAL);
                ERROR_CHECK(ret,-1,"send");
            ...
            {
                char * p = (char *)mmap(NULL,file_len,PROT_READ,MAP_SHARED,fd,0);
                ERROR_CHECK(p,(char *)-1,"mmap");
                ret = send(new_fd,p,file_len,MSG_NOSIGNAL);
                ERROR_CHECK(ret,-1,"send");
